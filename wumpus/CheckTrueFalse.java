...
public class CheckTrueFalse {
	...
	public static void main(String[] args) {
		...
		if( args.length != 3){
			...
			System.out.println("Usage: [program-name] [wumpus-rules-file] [additional-knowledge-file] [input_file]\n");
			exit_function(0);
		}
		...
		LogicalExpression not_knowledge_base = new LogicalExpression();
		not_knowledge_base.setSubexpression(knowledge_base);
		not_knowledge_base.setConnective("not");
		
		LogicalExpression not_statement = new LogicalExpression();
		not_statement.setSubexpression(statement);
		not_statement.setConnective("not");
		
		try {			
			BufferedWriter writer = new BufferedWriter(new FileWriter("results.txt"));
			boolean kbEntailsAlpha = entailmentChecker.TTEntails(knowledge_base, statement);
			boolean kbEntailsNotAlpha = entailmentChecker.TTEntails(knowledge_base, not_statement);
			String decision = "";
			if(kbEntailsAlpha && !kbEntailsNotAlpha) {
				decision = "definitely true";
			} else if(kbEntailsNotAlpha && !kbEntailsAlpha) {
